...
opponents. This paper presents CardShark-RL, a reinforcement
...
To answer this question, we design and implement CardShark-RL,
a complete RL training and evaluation pipeline. The system includes
a custom PettingZoo AEC environment for limit heads-up 5-Card Draw
Poker with three hand-coded opponent archetypes. Model~A (Explicit)
is a MaskablePPO agent that receives a one-hot encoding of the
opponent archetype in its observation vector. Model~B (Implicit)
receives only a rolling window of 10 behavioral statistics computed
from past hands, with no direct opponent identity signal. We further
apply Optuna-based hyperparameter optimization with a Rock-weighted
objective, reward shaping to encourage aggressive exploitation, and
a behavioral matrix analysis---``The Tell''---that reveals how
Model~B adapts its post-draw betting strategy based solely on
...
and proceeds through four phases. First, both players post a forced
before (ante) of 1~chip. Second, a pre-draw betting round takes place
using a small bet of 2~chips with up to 4~raises. Third, each player
selects 0--5 cards to discard and receives replacements from the deck.
Fourth, a post-draw betting round uses a big bet of 4~chips with up
to 4~raises, after which a showdown determines the winner by best
5-card hand. A player may win before showdown by causing the opponent
to fold.
...
The action space is \texttt{Discrete(35)}. Action~0 is Fold,
Action~1 is Call/Check, and Action~2 is Raise/Bet. Actions 3--34
are draw actions, where the index encodes a 5-bit bitmask over
the five card positions (bit $i = 1$ discards card~$i$).
Action~3 corresponds to bitmask~0 (stand pat; discard nothing)
and Action~34 corresponds to bitmask~31 (discard all five cards).
...
shared by both models are organized into six groups. Hand encoding
(10 features) represents each of the five cards as a normalized
rank and normalized suit, both in $[0,1]$. Hand strength
(2 features) gives a normalized hand category (0=High Card to
8=Straight Flush) and a normalized evaluation score. Pot features
(3 features) include normalized pot size, normalized bet-to-call,
and pot odds ratio. Phase encoding (3 features) is a one-hot vector
over $\{$pre-draw, draw, post-draw$\}$. Opponent draw count
(1 feature) is the normalized number of cards the opponent drew,
or $-1$ if the draw phase has not yet occurred. Strategic features
(4 features) cover a positional indicator, own raise count
(normalized), opponent aggression flag, and opponent pre-draw
raise flag.
...
computed over the last $W$ hands, comprising: fold frequency
pre-draw, fold frequency post-draw, raise frequency post-draw,
average cards drawn (normalized), raise-then-stand-pat correlation,
fold-after-drawing-3 frequency, raise-after-standing-pat frequency,
VPIP (Voluntarily Put In Pot), PFR (Pre-draw Raise frequency),
and Aggression Factor (raises / calls, normalized).
...
CallingStation (Archetype~0) is a passive opponent that never
...
Maniac (Archetype~1) is an aggressive opponent that raises
...
Rock (Archetype~2) is a tight-passive opponent that folds
...
Two shaping terms \cite{ng1999policy} are added. A fold penalty
$(-\alpha)$ is applied when the agent folds during a phase in which
checking would have been free (bet-to-call = 0), penalizing
unnecessarily surrendered equity. A steal bonus $(+\beta)$ is applied
when the agent raises and the opponent subsequently folds (a successful
steal), rewarding aggressive pot-theft. The shaping weights $\alpha$
and $\beta$ are treated as hyperparameters and tuned by Optuna.
For Model~B, the optimized values are $\alpha = 0.296$ and
$\beta = 0.448$.
